...
for (( i=1 ; i<=10; i++ ));  do 
ffmpeg -i "https://de2wa.com/video/input2.mp4"  -ss 00:00:0$(shuf -i 5-9 -n 1) -t 00:$(shuf -i 20-30 -n 1):00  -vbsf h264_mp4toannexb -f mpegts "input$i.ts"
done
...
for (( i=1 ; i<=50; i++ ));  do 
ffmpeg  -i "concat:intro$(shuf -i 1-10 -n 1).ts|introclick.ts|intro$(shuf -i 1-10 -n 1).ts|input$(shuf -i 1-10 -n 1).ts|input$(shuf -i 1-10 -n 1).ts|intro$(shuf -i 1-10 -n 1).ts|input$(shuf -i 1-10 -n 1).ts|input$(shuf -i 1-10 -n 1).ts" -i music.mp3 -c copy -map 0:v -map 1:a aajadi/kHGGhy$i.mp4
done
